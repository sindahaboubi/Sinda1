...
export class ListeServicesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.serviceSer.getArtistes().subscribe(artiste =>{
      for(let i=0;i<artiste['data'].length;i++){
        this.artistes.push(new Artiste((Number)(artiste['data'][i].id),
        artiste['data'][i].email,
        artiste['data'][i].password,
        artiste['data'][i].nom,
        artiste['data'][i].prenom,
        artiste['data'][i].dateNaissance,
        artiste['data'][i].genre,
        artiste['data'][i].etat,
        artiste['data'][i].image));
      }
    })
  ...
  onSubmit()
  {
    if(this.FormTri.value['artiste']!=null){
      var idArtiste=this.FormTri.value['artiste'];
      this.tabServiceFiltre=this.tabService.filter(service =>service.idArtiste==this.FormTri.value['artiste']);
    }
    ...
    this.FormTri.value['artiste']=idArtiste;
  }
